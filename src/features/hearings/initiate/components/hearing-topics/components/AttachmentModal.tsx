...
const AttachmentModal: React.FC<AttachmentModalProps> = ({
  ...
  const { data: WorkerAttachmentCategories, isFetching } =
    useWorkerAttachmentCategoriesQuery(
      {
        ModuleKey:
          userType === "Legal representative" || userType === "Establishment"
            ? "EstablishmentAttachmentCategories"
  ...
  const handleFileSelect = async (
    ...
    try {
      ...
      if (
        res.ServiceStatus === "Fail" &&
        Array.isArray(res.ErrorCodeList) &&
        res.ErrorCodeList.length > 0
      ) {
        return;
      }
  ...
  return (
    <>
      ...
      <Modal
        ...
        close={onClose}
        modalWidth={600}
        preventOutsideClick={isUploading}
      >
        <div
