...
import StepNavigation from "@/features/hearings/initiate/modules/case-creation/components/StepNavigation";
import { useNavigationService } from "@/shared/hooks/useNavigationService";
import { steps } from "@/features/hearings/initiate/modules/case-creation/components/tabs/tabsConfig";
import {
...
import { useGetRegionLookupDataQuery } from "@/features/hearings/initiate/api/create-case/workDetailApis";
...
function EditHearingTopicsDetails({
  ...
  const onSubmit = async (_data: TopicFormValues) => {
    ...
    try {
      ...
      if (result.success && result.response) {
        ...
        const hasNoErrors =
          !result.response?.ErrorCodeList ||
          result.response.ErrorCodeList.length === 0;
    ...
    const fetchCaseDetails = async () => {
      ...
      try {
        ...
        if (result.data?.CaseDetails) {
          ...
          if (
            statusWorkCode !== "Under-Negotiations" &&
        ...
        if (
          ...
          Array.isArray(error.data.ErrorDetails)
        ) {
  ...
  const handleNext = async (): Promise<{
    success: boolean;
    error?: any;
    response?: any;
  }> => {
              ...
              <div className="text-center">
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-500 mb-2">
                    {tManageHearing("redirecting_in_seconds", {
                      seconds: countdown,
                    }) || `Redirecting in ${countdown} seconds...`}
                  </p>
                  <p className="text-xs text-gray-400">
                    {tManageHearing("modal_will_close_automatically") ||
                      "This modal will close automatically and redirect you to the home page."}
                  </p>
                </div>
