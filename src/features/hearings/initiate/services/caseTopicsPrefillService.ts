import { formatDateString, formatHijriDate } from "@/utils/helpers";
...
export class CaseTopicsPrefillService {
  ...
  static extractDateFields(topic: CaseTopicData): CaseTopicDateFields {
    ...
    switch (topic.SubTopicID) {
      ...
      case "RLRAHI-1":
        if (
          topic.RequestType === "RLRAHI1" ||
          topic.RequestType_Code === "RLRAHI1"
        ) {
          if (topic.Date_New) {
  ...
  static extractFormFields(topic: CaseTopicData): Record<string, any> {
    ...
    const formFields = {
      ...
      refundAmount: topic.refundAmount || topic.Amount || topic.amount || "",
      compensationAmount:
        ...
        topic.amount ||
        "",
      wageAmount: topic.wageAmount || topic.Amount || topic.amount || "",
      noticeCompensationAmount: topic.Amount || topic.amount || "",
      endOfServiceRewardAmount: topic.Amount || topic.amount || "",
      rewardAmount: topic.Amount || topic.amount || "",
      rewardType: topic.RewardType || topic.rewardType || "",
      rewardRequestAmount: topic.Amount || topic.amount || "",
      bonusProfitShareAmount:
        topic.bonusProfitShareAmount || topic.Amount || topic.amount || "",
      ...
      durationOfLeaveDue:
        topic.DurationOfLeaveDue || topic.durationOfLeaveDue || "",
      ...
      currentInsuranceLevel:
        topic.CurrentInsuranceLevel || topic.currentInsuranceLevel || "",
      theReason: topic.Reason || topic.theReason || "",
      requiredDegreeOfInsurance:
        topic.RequiredDegreeInsurance || topic.requiredDegreeOfInsurance || "",
      ...
      typeOfRequest:
        topic.RequestType || topic.RequestType_Code || topic.TypeOfRequest
          ? {
              value:
                topic.RequestType_Code ||
                topic.RequestType ||
                topic.TypeOfRequest,
              label:
                topic.RequestType ||
      ...
      typesOfPenalties:
        topic.PenalityType || topic.PenalityType_Code || topic.TypesOfPenalties
          ? {
              value:
