...
import { isApiResponseSuccessful } from "@/utils/api/errorHandler";
...
const HearingDetails = () => {
  ...
  const handleCancelCase = async () => {
    try {
      ...
      if (isApiResponseSuccessful(cancelCaseResponse)) {
        ...
        handleResponse(cancelCaseResponse);
      }
    } catch {
  ...
  if (isError) {
    return (
      ...
      >
        ...
        <div className="bg-primary-600 h-[4px] rounded mx-4" />

        {/* Incomplete alert */}
        <div className="space-y-3xl">
          <Suspense fallback={<TableLoader />}>
            <div className="text-red-500 p-10 flex justify-center items-center">
              {t("error_loading_hearing")}
            </div>
          </Suspense>
        </div>
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="header_content bg-[#F7FDF9] flex flex-col gap-3 py-2 px-4 rounded-md">
        <div className=" flex items-center gap-2">
          <div className="flex items-center gap-2">
            <img src={auditIcon} alt="Hearing" className="w-6 h-6" />
            <div className="flex flex-col">
              <span className="text-md font-medium text-gray-600">
                {t("hearing_number")}
                <span className="text-md font-semibold text-primary-600 px-2">
                  ({caseId})
                </span>
              </span>
            </div>
        ...
        <div className="bg-primary-600 h-[4px] rounded" />
      </div>
      {/* Actions */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center px-4 py-4 gap-4">
      ...
      <div className="space-y-3xl">
        ...
        {statusCode === "Resolved-Rejected" && (
          <div className="bg-error-50 border-b-2 border-error-300 rounded-2xs px-6 py-2">
            <div className="flex items-center gap-2">
              <span className="bg-error-700 rounded-full w-6 h-6 flex items-center justify-center text-white font-bold text-xs">
                !
              </span>
      ...
      {/* Footer buttons */}
      <div className="border-t border-gray-200 pt-6 flex flex-col sm:flex-row justify-between mt-6 gap-4 px-2">
        <Button
          variant="secondary"
          size="xs"
          onClick={() => navigate("/manage-hearings")}
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          ...
          {editMode && (
            ...
            >
              {loadingAction === "update" ? t("saving") : t("save")}
            </Button>
          )}
          {!editMode && canResend && (
            <Button
        ...
        >
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <Button
              ...
              disabled={loadingAction === "reopen"}
            >
              {loadingAction === "reopen" ? t("submitting") : t("yes")}
            </Button>
          </div>
        </Modal>
      )}
