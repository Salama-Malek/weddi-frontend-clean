...
export function EmbassyPrincipalFormLayout({
  ...
  useEffect(() => {
    if (claimType === "principal" && getNicData?.NICDetails) {
      ...
      if (nic?.Region_Code)
        memoizedSetValue("region", {
          value: nic.Region_Code,
          label: nic.Region,
        });
      if (nic?.City_Code)
        memoizedSetValue("city", { value: nic.City_Code, label: nic.City });
      if (nic?.Occupation_Code)
        memoizedSetValue("occupation", {
          value: nic.Occupation_Code,
          label: nic.Occupation,
        });
      if (nic?.Gender_Code)
        memoizedSetValue("gender", {
          value: nic.Gender_Code,
          label: nic.Gender,
        });
      if (nic?.Nationality_Code)
        memoizedSetValue("nationality", {
          value: nic.Nationality_Code,
          label: nic.Nationality,
        });
      if (nic?.DateOfBirthHijri)
        memoizedSetValue("hijriDate", nic.DateOfBirthHijri);
      if (nic?.DateOfBirthGregorian)
        memoizedSetValue("gregorianDate", nic.DateOfBirthGregorian);
      if (nic?.Applicant) memoizedSetValue("applicant", nic.Applicant);
      if (nic?.PhoneNumber)
        memoizedSetValue("phoneNumber", nic.PhoneNumber.toString());
    }
  ...
  return [
    {
      ...
      children: [
            ...
            ]),
        {
          ...
          value:
            formatDateString(getNicData?.NICDetails?.DateOfBirthHijri) || "",
        },
        {
          type: "readonly" as const,
