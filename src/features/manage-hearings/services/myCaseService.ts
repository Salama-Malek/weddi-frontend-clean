...
} from "../api/myCasesApis";

import {
  PlaintiffCasesResponse,
  DefendantCasesResponse,
  CaseRecord,
} from "../types/myCases";
...
export const useMyCasesData = ({
  ...
  if (
    TableFor === "Plaintiff" &&
    (data as PlaintiffCasesResponse)?.PlaintiffCases
  ) {
    ...
    TableFor === "Defendant" &&
    (data as DefendantCasesResponse)?.DefendantCases
  ) {
