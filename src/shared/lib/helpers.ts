...
export function isArrayWithData(value: unknown): value is Array<unknown> {
...
export type OptionMapperParams = {
  data?: any[];
  valueKey?: string;
  labelKey?: string;
};
...
export const createAutocompleteField = ({
  ...
  type: getFieldType("autocomplete", readOnly),
  name,
  label,
  options,
  value,
...
export function formatDateGMT(inputDate: string | null | undefined): string {
  ...
  const year = inputDate.slice(0, 4),
    ...
    day = inputDate.slice(6, 8);
  const hour = inputDate.slice(9, 11),
    minute = inputDate.slice(11, 13);
